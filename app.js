...
async function fetchLiveData() {
    ...
    try {
        statusEl.textContent = 'Connecting to Robertson Pipeline…';
        
        let data;
        // 1. Try local server first (Development)
        try {
            const resp = await fetch('/api/data');
            if (!resp.ok) throw new Error('No local API');
            data = await resp.json();
            console.log('[APP] Data Source: Local API');
        } catch (err) {
            // 2. Fallback to Direct Static Fetch (Production/GitHub Pages)
            console.log('[APP] Local API unavailable, using Direct Static Fetch...');
            data = await fetchDirectFromFred();
            data.isStatic = true;
        }
        ...
        console.error('[APP] Global Load Error:', err);
        statusEl.textContent = `Connection error. Retrying in 10s…`;
        setTimeout(fetchLiveData, 10000);
    }
}

/**
 * Direct fetch from FRED using a CORS proxy (for Static Hosting like GitHub Pages)
 */
async function fetchDirectFromFred() {
    const series = { headline: 'CPIAUCSL', core: 'CPILFESL', trimmed: 'TRMMEANCPIM159SFRBCLE', fedfunds: 'FEDFUNDS' };
    const results = {};
    
    results.headlineLevel = await fetchCSV(series.headline, '2013-01-01');
    results.coreLevel = await fetchCSV(series.core, '2013-01-01');
    results.trimmed = await fetchCSV(series.trimmed, '2014-01-01');
    results.fedfunds = await fetchCSV(series.fedfunds, '2014-01-01');

    const hYoY = computeYoY(results.headlineLevel);
    const cYoY = computeYoY(results.coreLevel);

    return {
        timestamp: new Date().toISOString(),
        series: {
            headline: hYoY.filter(d => d.date >= '2014-01-01'),
            core: cYoY.filter(d => d.date >= '2014-01-01'),
            trimmed: results.trimmed,
            fedfunds: results.fedfunds
        },
        latest: {
            trimmedMean: results.trimmed[results.trimmed.length - 1],
            headlineCPI: hYoY[hYoY.length - 1],
            coreCPI: cYoY[cYoY.length - 1],
            fedFunds: results.fedfunds[results.fedfunds.length - 1]
        }
    };
}

async function fetchCSV(seriesId, start) {
    const url = `${FRED_PROXY}${encodeURIComponent(`${FRED_BASE}?id=${seriesId}&cosd=${start}&fq=Monthly`)}`;
    const resp = await fetch(url);
    if (!resp.ok) throw new Error(`FRED fetch failed: ${seriesId}`);
    const text = await resp.text();
    return text.trim().split('\n').slice(1).map(line => {
        const [date, val] = line.split(',');
        return { date: date.trim(), value: parseFloat(val.trim()) };
    }).filter(d => !isNaN(d.value) && d.date.match(/^\d{4}-\d{2}-\d{2}$/));
}

function computeYoY(levelData) {
    const byDate = {};
    levelData.forEach(d => byDate[d.date.slice(0, 7)] = d.value);
    return levelData.map(d => {
        const curDate = new Date(d.date + 'T00:00:00');
        ...
        if (byDate[priorKey]) {
            ...
            return { date: d.date, value: parseFloat(yoy.toFixed(2)) };
        }
        return null;
    }).filter(v => v !== null);
}
